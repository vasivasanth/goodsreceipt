...
document.getElementById('receiptDate').addEventListener('change', function () {
    var startDate = new Date(document.getElementById('receiptDate').value);
...
function addProductRow() {
    ...
    if (product != "" && (qty != "" && qty > 0) && (edate == "" || ed >= today) && !existingRow) {
        ...
        let changedFormatDate =  dateFormatChange(edate);
        console.log(edate);
...
function editRow(button) {
    ...
    let changedFormatDate =  dateFormatChange(td[2].innerText);
    const date = changedFormatDate;
...
function updateProductRow(event) {
    ...
    if (product != "" && (qty != "" && qty > 0) && (edate == "" || ed >= today) && !existingRow) {
        ...
        td[2].innerText = changedFormatDate;
        td[3].innerText = qty;
...
function dateFormatChange(receiptDate) {
    ...
    const changedFormatDate = parts[2] + '-' + parts[1] + '-' + parts[0];
...
function storeData() {
    ...
    let changedFormatDate = dateFormatChange(receiptDate);

    if (check & productTable.innerHTML !== "") {
        const newReceipt = {
            receiptID: receiptId,
            receiptDate: changedFormatDate,
...
function listEditRow(button) {
    ...
    let receiptDateReversed = dateFormatChange(row.cells[2].innerText.trim());
    console.log(dateFormatChange(row.cells[2].innerText.trim()));

    document.getElementById('receiptId').value = row.cells[1].innerText;
    document.getElementById('receiptDate').value = receiptDateReversed;
...
function updateData() {
    ...
    if (check & productTable.innerHTML !== "") {
        ...
        updateParentRow.cells[2].innerText = changedFormatDate;
